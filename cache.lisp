      ...
      (let* ((key (cons (string-downcase (rr-name rr)) (rr-type rr)))
        ...
        (if existing
            (setf (car existing) (max (car existing) expiry)
                  (cdr existing) (cons rr (cdr existing)))
...
(defun cache-load (cache pathname)
  ...
  (when (probe-file pathname)
    (with-open-file (in pathname :direction :input)
      ;; Bound iteration to guard against oversized/malformed cache files.
      ;; *read-eval* disabled — cache files are untrusted input.
      (let ((*read-eval* nil))
      (loop with max-entries = (* 10 (cache-max-size cache))
            repeat max-entries
            for entry = (read in nil :eof)
            until (eq entry :eof)
